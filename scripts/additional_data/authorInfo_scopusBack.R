...
if( file.exists("startnr.csv" ) ) {
  ...
  startnr <- startnr[1,1]
} else { startnr <- length(df[,1]) }
maximum <- 1
progbar <- txtProgressBar( min = 0, max = startnr, style = 3 )
...
write.csv2( df, "/mnt/data/live02/stress/hlamberink/csv/data/authors_with_detailsBack.csv", row.names = F )
