...
# read data of new pmids
news <- read.csv2( "pdfNEW/new_rcts.csv", header = F, stringsAsFactors = F )
names(news) <- c( "pmid", "url" )
print( length( news$pmid ) )

# set progress bar
progbar <- txtProgressBar( min = 0, max = length( news$pmid ), style = 3 )


# for every pmid, add url
for( i in 1:length( news$pmid ) )
{
  setTxtProgressBar( progbar, i )
  
  # add url
  pp <- data.frame( pmid = news$pmid[ i ],
                    url = news$url[ i ],
