...
setwd( "/mnt/cortexraid/hlamberink/csv" ) 
...
# loop over sets of 20000 pmids and write datafiles (22x)
for( i in 1:3 )
{ 
  d <- i * 20000
  e <- d - 20000
  ...
  startnr <- e
  print( paste( "original startnr: ", e ) )
  
  
  if( file.exists( paste0( "csvout/pubmeddata_limited", i, ".csv" ) ) )
  {
    a <- read.csv2( paste0( "csvout/pubmeddata_limited", i, ".csv" ) )
  ...
  # skip file if the file is complete (length = 20 in this case)
  if( startnr == f + 1 ){ print("go to next file"); next } 
  ...
  # prepare files
  if( !file.exists( paste0( "csvout/pubmeddata", i, ".csv" ))) 
  {
    write.table( noabs.file, paste0( "csvout/pubmeddata", i, ".csv" ), 
                 sep = ";", dec = ",", row.names = FALSE, col.names = FALSE, append = FALSE )
  }
  if( !file.exists( paste0( "csvout/pubmeddata_limited", i, ".csv" ))) 
  {
    write.table( small.file, paste0( "csvout/pubmeddata_limited", i, ".csv" ), 
                 sep = ";", dec = ",", row.names = FALSE, col.names = FALSE, append = FALSE )
  }
  if( !file.exists( paste0( "csvout/pubmeddata_tiab", i, ".csv" ))) 
  {
    write.table( tiab.file, paste0( "csvout/pubmeddata_tiab", i, ".csv" ), 
                 sep = ";", dec = ",", row.names = FALSE, col.names = FALSE, append = FALSE )
  }
  ...
  for( b in startnr:f )
  {
    ...
    if( b %in% seq(0,20000,50) ) Sys.sleep( rnorm(1, mean=60, sd=10 ) ) 
    
    # update progressbar
    setTxtProgressBar( progbar, b )
    
    # get xml data from pubmed database
    pmid <- pmidlist$pmids[ b ]
    ...
    # write data
    write.table( theData[ , noabs.file ], paste0( "csvout/pubmeddata", i, ".csv" ), 
                 sep = ";", dec = ",", row.names = FALSE, col.names = FALSE, append = TRUE )
    write.table( theData[ , small.file ], paste0( "csvout/pubmeddata_limited", i, ".csv" ), 
                 sep = ";", dec = ",", row.names = FALSE, col.names = FALSE, append = TRUE )
    write.table( theData[ , tiab.file ], paste0( "csvout/pubmeddata_tiab", i, ".csv" ), 
                 sep = ";", dec = ",", row.names = FALSE, col.names = FALSE, append = TRUE )
  } 
}
