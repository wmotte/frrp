...
# read data
ascopubs <- read.csv2( "missingascopubs.csv", stringsAsFactors = FALSE )


# download pdfs
# set progress bar
progbar <- txtProgressBar( min = 0, max = length( ascopubs$pmid ), style = 3 )
...
for( i in 1:length( ascopubs$pmid ) )
{
  setTxtProgressBar( progbar, i )
  
  # add url
  pp <- data.frame( pmid = ascopubs$pmid[ i ],
                    url = ascopubs$urlfull[ i ],
                    stringsAsFactors = FALSE )
  get.pdf( pp$pmid )
  
  
  # print elapsed time
  if( i == length( ascopubs$pmid ) ) 
  {
