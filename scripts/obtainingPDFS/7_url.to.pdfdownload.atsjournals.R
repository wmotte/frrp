...
# read data
atsjournals <- read.csv2( "missingatsjournals.csv", stringsAsFactors = FALSE )


# download pdfs
# set progress bar
progbar <- txtProgressBar( min = 0, max = length( atsjournals$pmid ), style = 3 )
...
for( i in 1:length( atsjournals$pmid ) )
{
  setTxtProgressBar( progbar, i )
  
  # add url
  pp <- data.frame( pmid = atsjournals$pmid[ i ],
                    url = atsjournals$urlfull[ i ],
                    stringsAsFactors = FALSE )
  get.pdf( pp$pmid )
  
  
  # print elapsed time
  if( i == length( atsjournals$pmid ) ) 
  {
