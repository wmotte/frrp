...
##
get.pdf <- function( pmid, outdr = outdir, df = p  )
{
...
get.pdflink.bmj <- function( url )
{
  ...
  # css of pdf element selected with Selector Gadget Google Chrome plugin [http://selectorgadget.com/] 
  css <- ".pane-content .pdf-link"
  
  # get href to pdfLink
  pdflink <- page %>% html_nodes( css = css ) %>%  html_attr( "href" )
  pdflink <- paste0( "http://www.bmj.com", pdflink )
  
  if( pdflink == "http://www.bmj.com" )
  {
      css2 <- 'meta[name="citation_pdf_url"]'
      pdflink <- page %>% html_nodes( css = css2 ) %>% html_attr( "content" )
  }
...
get.pdflink.nejm <- function( url )
{
  ...
  # css of pdf element selected with Selector Gadget Google Chrome plugin [http://selectorgadget.com/] 
  css <- ".zone-tools-articlePdf"
  
  # get href to pdfLink
  pdflink <- page %>% html_nodes( css = css ) %>%  html_attr( "href" )
  pdflink <- paste0( "http://www.nejm.org", pdflink )
...
get.pdflink.acoup <- function( url )
{
  ...
  # get href to pdfLink
  pdflink <- page %>% html_nodes( css = css ) %>%  html_attr( "href" )
  pdflink <- paste0( "https://academic.oup.com", pdflink )
...
get.pdflink.ovid <- function( url )
{
  ...
  page2 <- xml2::read_html( curl( p6, handle = curl::new_handle( "useragent" = "Chrome/55.0" ) ) )
  intermed1 <- page2 %>% html_nodes( css = "#pdf" ) %>% html_attr( "href" )
...
###
# Get full text pdf link from gutnliver full text website.
##
get.pdflink.gutnliver <- function( url )
{ 
  ###TODO
  # retrieve pmid
  # paste0( "https://www.ncbi.nlm.nih.gov/pubmed/?term=", pmid )
  # use UBU link
} 

###
# Get full text pdf link from jcimjournal full text website.
##
get.pdflink.jcimjournal <- function( url )
{ 
  # try via sciencedirect or UBU
} 

###
# Get full text pdf link from neurology full text website.
##
get.pdflink.neurology <- function( url )
{ 
  ###TODO
  # retrieve pmid
  # paste0( "https://www.ncbi.nlm.nih.gov/pubmed/?term=", pmid )
...
###
# Get full text pdf link from psychiatrist.com full text website.
##
get.pdflink.psychiatrist.com <- function( url )
{ 
  # HTTP ERROR 403

  # parse page
#  page <- xml2::read_html( curl( url, handle = curl::new_handle( "useragent" = "Safari/419.3" ) ) )
  
  # css of pdf element selected with Selector Gadget Google Chrome plugin [http://selectorgadget.com/] 
#  css <- '#linkSpan :nth-child(1) a' 
  
  # get href to pdfLink
#  pdflink <- page %>% html_nodes( css = css )
#  %>% html_attr( "content" )
  
}

###
# Get full text pdf link from UBUlink full text website.
##
get.pdflink.ubulink <- function( url )
{ 
  # retrieve pmid
  #pmid <- p[ p$url == url & !is.na( p$url) , "pmid" ]
  
  # first link to publisher
  #intermed1 <- paste0( "http://sfx.library.uu.nl/utrecht?sid=Entrez:PubMed&id=pmid:", pmid )
  
  
  # parse page
  #page <- xml2::read_html( curl( intermed1, handle = curl::new_handle( "useragent" = "Chrome/55.0" ) ) )
  
  # css of pdf element selected with Selector Gadget Google Chrome plugin [http://selectorgadget.com/] 
  #css <- '#tr_1_basic1 a' 
  
  # get href to pdfLink
  #intermed2 <- page %>% html_nodes( css = css ) %>% html_attr( "href" )
...
###
# Get full text pdf link from yiigle full text website.
##
get.pdflink.yiigle <- function( url )
{ 
    # chinese
} 

###
# Get full text pdf link from nutricionhospitalaria.net full text website.
##
get.pdflink.nutricionhospitalaria <- function( url )
{ 
    # spanish
} 
...
###
# Get full text pdf link from cfcp full text website.
##
get.pdflink.cfcp <- function( url )
{ 
  # chinese
} 
...
###
# Get full text pdf link from clinsci.org full text website.
##
get.pdflink.clinsci <- function( url )
{ 
  # parse page
  #page <- xml2::read_html( curl( url, handle = curl::new_handle( "useragent" = "Chrome/55.0" ) ) )
  ...
  # get href to pdfLink
  #intermed1 <- page %>% html_nodes( css = css ) %>% html_attr( "href" )
  #pdflink <- paste0( "http://clinsci.org", intermed1 ) 
...
###
# Get full text pdf link from iospress.nl full text website.
##
get.pdflink.iospress <- function( url )
{
  # parse page
  #page <- xml2::read_html( curl( url, handle = curl::new_handle( "useragent" = "Chrome/55.0" ) ) )
  
  # css of pdf element selected with Selector Gadget Google Chrome plugin [http://selectorgadget.com/] 
  #css <- '#downloadlink46147'
...
###
# Get full text pdf link from aaiddjournals full text website.
##
get.pdflink.aaidd <- function( url )
{
    # parse page
    #page <- xml2::read_html( curl( url, handle = curl::new_handle( "useragent" = "Chrome/55.0" ) ) )
    ...
    # css of pdf element selected with Selector Gadget Google Chrome plugin [http://selectorgadget.com/] 
    # only modification is "\" before the double quotes.
    #css <- "#articleToolsFormats li+ li a"
    
    # get href to pdfLink
    #intermed1 <- page %>% html_nodes( css = css ) %>%  html_attr( "href" )
    #pdflink <- paste0( "http://aaiddjournals.org", intermed1)
...
###
# Get full text pdf link from ajcc.aacnjournals.org full text website.
##
get.pdflink.ajcc <- function( url )
{
    # parse page
    #page <- xml2::read_html( curl( url, handle = curl::new_handle( "useragent" = "Chrome/55.0" ) ) )
...
###
# Get full text pdf link from uwpress.org full text website.
##
get.pdflink.uwpress <- function( url )
{
  ###
}

###
# Get full text pdf link from apa.org full text website.
##
get.pdflink.apa.org <- function( url )
{
  ###
}
...
###
# Get full text pdf link from hogrefe.com full text website.
##
get.pdflink.hogrefe <- function( url )
{ 
    # parse page
    #page <- xml2::read_html( curl( url, handle = curl::new_handle( "useragent" = "Chrome/55.0" ) ) )
...
###
# Get full text pdf link from guilfordjournals full text website.
##
get.pdflink.guilfordjournals <- function( url )
{
    ###
}

###
# Get full text pdf link from jddonline full text website.
##
get.pdflink.jddonline <- function( url )
{ 
    # parse page
    page <- xml2::read_html( curl( url, handle = curl::new_handle( "useragent" = "Chrome/55.0" ) ) )
...
###
# Get full text pdf link from jocpd.org full text website.
##
get.pdflink.jocpd <- function( url )
{ 
    # parse page
    #page <- xml2::read_html( curl( url, handle = curl::new_handle( "useragent" = "Chrome/55.0" ) ) )
...
###
# Get full text pdf link from endocrinology-journals full text website.
##
get.pdflink.apa.endocrinologyjournals <- function( url )
{
    ###
}

###
# Get full text pdf link from aace full text website.
##
get.pdflink.aace <- function( url )
{ 
    # parse page
    #page <- xml2::read_html( curl( url, handle = curl::new_handle( "useragent" = "Chrome/55.0" ) ) )
...
###
# Get full text pdf link from humankinetics full text website.
##
get.pdflink.apa.humankinetics <- function( url )
{
    ###
}

###
# Get full text pdf link from sagepub full text website.
##
get.pdflink.sagepub <- function( url )
{ 
    # parse page
    #page <- xml2::read_html( curl( url, handle = curl::new_handle( "useragent" = "Chrome/55.0" ) ) )
    ...
    # get href to pdfLink
    #intermed1 <- page %>% html_nodes( css = css ) %>% html_attr( "href" )
    #pdflink <- paste0( "http://journals.sagepub.com", intermed1[1] )
    
    #return( pdflink )
} 

###
# Get full text pdf link from utpjournals full text website.
##
get.pdflink.utpjournals <- function( url )
{ 
    ## no full access, veterinary teaching
} 

###
# Get full text pdf link from jhu.edu full text website.
##
get.pdflink.jhu <- function( url )
{ 
    # parse page
    #page <- xml2::read_html( curl( url, handle = curl::new_handle( "useragent" = "Chrome/55.0" ) ) )
...
###
# Get full text pdf link from psychiatryonline full text website.
##
...
###
# Get full text pdf link from store.ons.org full text website.
##
get.pdflink.store.ons.org <- function( url )
{ 
    ## no full access
} 

###
# Get full text pdf link from aafp full text website.
##
get.pdflink.aafp <- function( url )
{ 
    ## no full access
} 

###
# Get full text pdf link from aaic.net.au full text website.
##
get.pdflink.aaic.net.au <- function( url )
{ 
    ## no full access
} 

###
# Get full text pdf link from ajph.org full text website.
##
get.pdflink.ajph <- function( url )
{ 
    ## no full access
} 

###
# Get full text pdf link from ajtmh.org full text website.
##
get.pdflink.ajtmh <- function( url )
{ 
    ## no full access
} 

###
# Get full text pdf link from akademiai.com full text website.
##
get.pdflink.akademiai <- function( url )
{ 
    ## no full access, Hungarian
} 

###
# Get full text pdf link from anesthesiaprogress.org full text website.
##
get.pdflink.anesthesiaprogress <- function( url )
{ 
    ## no full access
} 

###
# Get full text pdf link from clinchem.org full text website.
##
get.pdflink.clinchem <- function( url )
{ 
  ## no full access
} 

###
# Get full text pdf link from clinexprheumatol.org full text website.
##
get.pdflink.clinexprheumatol <- function( url )
{ 
  ## no full access
} 

###
# Get full text pdf link from dustri full text website.
##
get.pdflink.dustri <- function( url )
{ 
  ## no full access
} 

###
# Get full text pdf link from elsevier.es full text website.
##
get.pdflink.elsevier.es <- function( url )
{ 
  ## no full access
} 

###
# Get full text pdf link from eur-j-ophthalmol full text website.
##
get.pdflink.eurjophthalmol <- function( url )
{ 
  ## no full access
} 

###
# Get full text pdf link from eurekaselect full text website.
##
get.pdflink.eurekaselect <- function( url )
{ 
  ## no full access
} 

###
# Get full text pdf link from futuremedicine full text website.
##
get.pdflink.futuremedicine <- function( url )
{ 
  ## no full access
} 

###
# Get full text pdf link from future-science full text website.
##
get.pdflink.futurescience <- function( url )
{ 
  ## no full access
} 

###
# Get full text pdf link from hkmj full text website.
##
get.pdflink.hkmj <- function( url )
{ 
  ## no full access
} 

###
# Get full text pdf link from hospitalpediatrics full text website.
##
get.pdflink.hospitalpediatrics <- function( url )
{ 
  ## no full access
} 

###
# Get full text pdf link from ijdr full text website.
##
get.pdflink.ijdr <- function( url )
{ 
  # no full text
} 
...
###
# Get full text pdf link from ijp-online full text website.
##
get.pdflink.ijponline <- function( url )
{ 
  # no full access
} 

###
# Get full text pdf link from invasivecardiology full text website.
##
get.pdflink.invasivecardiology <- function( url )
{ 
  # no full access
} 

###
# Get full text pdf link from jdentaled full text website.
##
get.pdflink.jdentaled <- function( url )
{ 
  # no full access
} 

###
# Get full text pdf link from john-libbey-eurotext full text website.
##
get.pdflink.johnlibbeyeurotext <- function( url )
{ 
  # no full access
} 

###
# Get full text pdf link from medscimonit full text website.
##
get.pdflink.medscimonit <- function( url )
{ 
  # parse page
  #page <- xml2::read_html( curl( url, handle = curl::new_handle( "useragent" = "Chrome/55.0" ) ) )
  ...
  # get href to pdfLink
  #intermed1 <- page %>% html_nodes( css = css ) %>% html_attr( "action" )
...
p <- read.csv2( "pdf/all_rcts.csv", stringsAsFactors = FALSE, header = FALSE, skip = 200000, nrows = 50000 )
names(p) <- c( "pmids", "url" )
...
write.csv2( missings, "missings_5.csv" )