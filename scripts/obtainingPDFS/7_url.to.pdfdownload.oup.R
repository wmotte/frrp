...
# read data
oup <- read.csv2( "missingoup.csv", stringsAsFactors = FALSE )

# download pdfs
# set progress bar
progbar <- txtProgressBar( min = 0, max = length( oup$pmid ), style = 3 )
...
for( i in 1:length( oup$pmid ) )
{
  setTxtProgressBar( progbar, i )
  
  # add url
  pp <- data.frame( pmid = oup$pmid[ i ],
                    url = oup$urlfull[ i ],
