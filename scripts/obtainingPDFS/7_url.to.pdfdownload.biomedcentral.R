...
# read data
biomedcentral <- read.csv2( "missingbiomedcentral.csv", stringsAsFactors = FALSE )

# download pdfs
# set progress bar
progbar <- txtProgressBar( min = 0, max = length( biomedcentral$pmid ), style = 3 )
...
for( i in 1:length( biomedcentral$pmid ) )
{
  setTxtProgressBar( progbar, i )
  
  # add url
  pp <- data.frame( pmid = biomedcentral$pmid[ i ],
                    url = biomedcentral$urlfull[ i ],
                    stringsAsFactors = FALSE )
  get.pdf( pp$pmid )
  
  
  # print elapsed time
  if( i == length( biomedcentral$pmid ) ) 
  {
