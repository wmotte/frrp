...
# read data
nejm <- read.csv2( "missingnejm.csv", stringsAsFactors = FALSE )


# download pdfs
# set progress bar
progbar <- txtProgressBar( min = 0, max = length( nejm$pmid ), style = 3 )
...
for( i in 1:length( nejm$pmid ) )
{
  setTxtProgressBar( progbar, i )
  
  # add url
  pp <- data.frame( pmid = nejm$pmid[ i ],
                    url = nejm$urlfull[ i ],
