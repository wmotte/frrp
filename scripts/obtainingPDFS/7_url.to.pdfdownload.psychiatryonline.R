...
# read data
psychiatryonline <- read.csv2( "missingpsychiatryonline.csv", stringsAsFactors = FALSE )


# download pdfs
# set progress bar
progbar <- txtProgressBar( min = 0, max = length( psychiatryonline$pmid ), style = 3 )
...
for( i in 1:length( psychiatryonline$pmid ) )
{
  setTxtProgressBar( progbar, i )
  
  # add url
  pp <- data.frame( pmid = psychiatryonline$pmid[ i ],
                    url = psychiatryonline$urlfull[ i ],
                    stringsAsFactors = FALSE )
  get.pdf( pp$pmid )
  
  
  # print elapsed time
  if( i == length( psychiatryonline$pmid ) ) 
  {
