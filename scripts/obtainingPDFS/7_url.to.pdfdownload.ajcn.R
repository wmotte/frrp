...
# read data
ajcn <- read.csv2( "missingajcn.csv", stringsAsFactors = FALSE )

# download pdfs
# set progress bar
progbar <- txtProgressBar( min = 0, max = length( ajcn$pmid ), style = 3 )
...
for( i in 1:length( ajcn$pmid ) )
{
  setTxtProgressBar( progbar, i )
  
  # add url
  pp <- data.frame( pmid = ajcn$pmid[ i ],
                    url = ajcn$urlfull[ i ],
