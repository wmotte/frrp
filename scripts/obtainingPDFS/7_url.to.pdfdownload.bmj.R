...
get.pdflink.bmj <- function( url )
{
  ...
  css <- ".article-pdf-download"
  
  # get href to pdfLink
  intermed1 <- page %>% html_nodes( css = css ) %>%  html_attr( "href" )
  link <- strsplit( url, ".com" )
  pdflink <- paste0( link[[1]][1], ".com", intermed1 )
...
# read data
bmj <- read.csv2( "missingbmj.csv", stringsAsFactors = FALSE )

# download pdfs
# set progress bar
progbar <- txtProgressBar( min = 0, max = length( bmj$pmid ), style = 3 )
...
for( i in 1:length( bmj$pmid ) )
{
  setTxtProgressBar( progbar, i )
  
  # add url
  pp <- data.frame( pmid = bmj$pmid[ i ],
                    url = bmj$urlfull[ i ],
