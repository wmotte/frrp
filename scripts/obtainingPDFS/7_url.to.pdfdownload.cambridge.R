...
# read data
cambridge <- read.csv2( "missingcambridge.csv", stringsAsFactors = FALSE )

# download pdfs
# set progress bar
progbar <- txtProgressBar( min = 0, max = length( cambridge$pmid ), style = 3 )
...
for( i in 1:length( cambridge$pmid ) )
{
  setTxtProgressBar( progbar, i )
  
  # add url
  pp <- data.frame( pmid = cambridge$pmid[ i ],
                    url = cambridge$urlfull[ i ],
                    stringsAsFactors = FALSE )
  get.pdf( pp$pmid )
  
  
  # print elapsed time
  if( i == length( cambridge$pmid ) ) 
  {
