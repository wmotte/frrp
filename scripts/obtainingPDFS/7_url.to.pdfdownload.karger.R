...
# read data
karger <- read.csv2( "missingkarger.csv", stringsAsFactors = FALSE )

# download pdfs
# set progress bar
progbar <- txtProgressBar( min = 0, max = length( karger$pmid ), style = 3 )
...
for( i in 1:length( karger$pmid ) )
{
  setTxtProgressBar( progbar, i )
  
  # add url
  pp <- data.frame( pmid = karger$pmid[ i ],
                    url = karger$urlfull[ i ],
                    stringsAsFactors = FALSE )
  get.pdf( pp$pmid )
  
  
  # print elapsed time
  if( i == length( karger$pmid ) ) 
  {
