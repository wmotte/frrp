...
# read data
tandfonline <- read.csv2( "missingtandfonline.csv", stringsAsFactors = FALSE )

# download pdfs
# set progress bar
progbar <- txtProgressBar( min = 0, max = length( tandfonline$pmid ), style = 3 )
...
for( i in 1:length( tandfonline$pmid ) )
{
  setTxtProgressBar( progbar, i )
  
  # add url
  pp <- data.frame( pmid = tandfonline$pmid[ i ],
                    url = tandfonline$urlfull[ i ],
                    stringsAsFactors = FALSE )
  get.pdf( pp$pmid )
  
  
  # print elapsed time
  if( i == length( tandfonline$pmid ) ) 
  {
