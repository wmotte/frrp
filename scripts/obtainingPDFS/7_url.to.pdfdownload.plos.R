...
# read data
plos <- read.csv2( "missingplos.csv", stringsAsFactors = FALSE )

# download pdfs
# set progress bar
progbar <- txtProgressBar( min = 0, max = length( plos$pmid ), style = 3 )
...
for( i in 1:length( plos$pmid ) )
{
  setTxtProgressBar( progbar, i )
  
  # add url
  pp <- data.frame( pmid = plos$pmid[ i ],
                    url = plos$urlfull[ i ],
