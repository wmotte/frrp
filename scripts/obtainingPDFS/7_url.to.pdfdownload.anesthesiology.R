...
# read data
anesthesiology <- read.csv2( "missinganesthesiology.csv", stringsAsFactors = FALSE )


# download pdfs
# set progress bar
progbar <- txtProgressBar( min = 0, max = length( anesthesiology$pmid ), style = 3 )
...
for( i in 1:length( anesthesiology$pmid ) )
{
  setTxtProgressBar( progbar, i )
  
  # add url
  pp <- data.frame( pmid = anesthesiology$pmid[ i ],
                    url = anesthesiology$urlfull[ i ],
                    stringsAsFactors = FALSE )
  get.pdf( pp$pmid )
  
  
  # print elapsed time
  if( i == length( anesthesiology$pmid ) ) 
  {
