...
# read data
asm <- read.csv2( "missingasm.csv", stringsAsFactors = FALSE )


# download pdfs
# set progress bar
progbar <- txtProgressBar( min = 0, max = length( asm$pmid ), style = 3 )
...
for( i in 1:length( asm$pmid ) )
{
  setTxtProgressBar( progbar, i )
  
  # add url
  pp <- data.frame( pmid = asm$pmid[ i ],
                    url = asm$urlfull[ i ],
