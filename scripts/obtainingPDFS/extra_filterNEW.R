...
pmidlist <- read.csv2( "new_rcts_pmidsonly_2018-03-19.csv", stringsAsFactors = FALSE )


# loop over 1 datasets, afterward summarize the total
for( i in 23 )
{
  ...
  #write.csv2( languages, paste0( outdata, "/excluded_languagesNEW.csv" ), row.names = FALSE )
...
#exclLang <- read.csv2( paste0( outdata, "/excluded_languagesNEW.csv" ), stringsAsFactors = FALSE )
#exclLang$sum <- rowSums(exclLang[, -1], na.rm = T)
#exclLang$perc <- round( exclLang$sum / sum(exclLang$sum), digits = 5 )
#write.csv2( exclLang, paste0( outdata, "/excluded_languagesNEW.csv" ), row.names = FALSE )
...
# loop over 23 files of selected pmids and bind into one vector
for( i in 1:23 )
{
...
for( i in 1:23 )
{
  ...
  if( i == 23 )
  {
    ...
    df <- read.csv2( paste0( "csvout/pubmeddata_tiab", i, "_new.csv" ), stringsAsFactors = FALSE, quote = "" )
  }
  ...
  if( i == 23 )
  {
    ...
    df2 <- read.csv2( paste0( "csvout/pubmeddata", i, ".csv" ), stringsAsFactors = FALSE, quote = "" )
  }
...
# write to file
write.csv2( cochraneSummary, paste0( outdata, "/cochrane_summaryNEW.csv" ), row.names = FALSE )
write.csv2( cochrane_excluded, paste0( outdata, "/cochrane_excludedNEW.csv" ), row.names = FALSE )
