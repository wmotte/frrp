...
# read data
p <- read.csv2( "url_NA.csv", stringsAsFactors = FALSE, skip = 60000, nrows = 20000,
...
if( file.exists( paste0( outdir, '/all_rctsNA4.csv' ) ) )
{
  a <- read.csv2(paste0( outdir, '/all_rctsNA4.csv' ) )
...
for( i in startnr:length( p$pmid ) )
{
  ...
  # write to file
  write.table( pp, file = paste0( outdir, '/all_rctsNA4.csv' ), 
