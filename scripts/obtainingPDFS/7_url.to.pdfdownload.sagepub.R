  ...
  v <- tryCatch(
    {
      ...
      if( !file.exists( outpdf ) )
      {
        ...
        #######################
        pdflink <- get.pdflink.sagepub( url )
...
get.pdflink.sagepub <- function( url )
{
  ...
  css <- 'a[data-item-name="download-PDF"]'
  
  # get href to pdfLink
  intermed <- page %>% html_nodes( css = css ) %>% html_attr( "href" )
  pdflink <- paste0( "http://journals.sagepub.com", intermed)
...
# read data
sagepub <- read.csv2( "missingsagepub.csv", stringsAsFactors = FALSE )

# download pdfs
# set progress bar
progbar <- txtProgressBar( min = 0, max = length( sagepub$pmid ), style = 3 )
...
for( i in 1:length( sagepub$pmid ) )
{
  setTxtProgressBar( progbar, i )
  
  # add url
  pp <- data.frame( pmid = sagepub$pmid[ i ],
                    url = sagepub$urlfull[ i ],
                    stringsAsFactors = FALSE )
  get.pdf( pp$pmid )
  
  
  # print elapsed time
  if( i == length( sagepub$pmid ) ) 
  {
