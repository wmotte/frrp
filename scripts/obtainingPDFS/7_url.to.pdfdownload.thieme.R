...
# read data
thieme <- read.csv2( "missingthieme.csv", stringsAsFactors = FALSE )


# download pdfs
# set progress bar
progbar <- txtProgressBar( min = 0, max = length( thieme$pmid ), style = 3 )
...
for( i in 1:length( thieme$pmid ) )
{
  setTxtProgressBar( progbar, i )
  
  # add url
  pp <- data.frame( pmid = thieme$pmid[ i ],
                    url = thieme$urlfull[ i ],
                    stringsAsFactors = FALSE )
  get.pdf( pp$pmid )
  
  
  # print elapsed time
  if( i == length( thieme$pmid ) ) 
  {
