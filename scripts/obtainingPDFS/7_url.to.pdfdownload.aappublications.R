...
# read data
aappublications <- read.csv2( "missingaappublications.csv", stringsAsFactors = FALSE )


# download pdfs
# set progress bar
progbar <- txtProgressBar( min = 0, max = length( aappublications$pmid ), style = 3 )
...
for( i in 1:length( aappublications$pmid ) )
{
  setTxtProgressBar( progbar, i )
  
  # add url
  pp <- data.frame( pmid = aappublications$pmid[ i ],
                    url = aappublications$urlfull[ i ],
                    stringsAsFactors = FALSE )
  get.pdf( pp$pmid )
  
  
  # print elapsed time
  if( i == length( aappublications$pmid ) ) 
  {
